...
export default async function (
	context: Context<"issues.labeled" | "issues.unlabeled">,
) {
	...
	console.info(
		`#${issue.number} ${isUnlabeled ? "unlabeled" : "labeled"}: ${label.name} (${labelId})`,
	);
	// check sender type
	...
	if (Labels.isPositiveLabel(labelId)) {
		for (const l of issueLabelIds)
			if (Labels.isNegativeLabel(l)) labelsToRemove.push(l);
		if (
			(issue.state === "open" || issue.state_reason !== "completed") &&
			Labels.isDoneLabel(labelId)
		) {
	...
	// negative -> remove all labels except self and markup; reopen the issue, or close as not planned
		...
		for (const l of issueLabelIds)
			if (l !== labelId && !Labels.isMarkupLabel(l)) labelsToRemove.push(l);
	...
	if (labelsToRemove.length > 0) {
		...
		for (const n of labelNames)
			await octokit.issues.removeLabel(context.issue({ name: n }));
	}
}
