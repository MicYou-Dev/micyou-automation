...
export const prAll = async (context: Context<"pull_request">) =>
	await pr(context, false);
export const prReview = async (context: Context<"pull_request_review">) =>
	await pr(context, true);
...
async function pr(
	...
	// count changing size
	if (isReviewEvent) {
		const labels = (await octokit.issues.listLabelsOnIssue(context.issue()))
			.data;
		const sizeLabelIndex = labels.findIndex((l) => Labels.isSizeLabel(l.id));
		if (sizeLabelIndex !== -1) {
			labelsToSet.push(labels[sizeLabelIndex].id);
			console.debug(
				`Size label: ${labels[sizeLabelIndex].name} // f* you GitHub API`,
			);
		}
	} else {
		...
		console.debug(
			`Changes: ${changes} (additions: ${prc.additions}, deletions: ${prc.deletions})`,
		);
...
async function checkMergeable(context: Context<"pull_request">) {
	...
	return pr.reviewDecision === "APPROVED" && pr.mergeable === "MERGEABLE";
}

async function markReferencedIssues(context: Context, labelId: number) {
	const octokit = context.octokit.rest;
	const referencedIssues = await getClosingIssuesReferences(
		context.octokit,
		context.issue(),
	);
		...
		for (const issueNumber of referencedIssues) {
			...
			await octokit.issues.addLabels(
				context.repo({
					...
					labels: await context.label(self),
				}),
			);
			...
			for (const l of labelNames)
				await octokit.issues.removeLabel(
					context.repo({ issue_number: issueNumber, name: l }),
				);
		}
	}
}
