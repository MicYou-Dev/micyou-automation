...
export default async function (context: Context<"issue_comment">) {
	...
	console.info(
		`#${issue.number} comment: ${comment.id} ${action} by '${sender}'`,
	);
	if (action === "deleted") {
	...
	const {
		repository: {
			targetIssue: {
				id: targetIssueId,
				stateReason,
				duplicateOf: currentDuplicateOf,
			},
